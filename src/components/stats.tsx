    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true)
          const animateCount = (target: number, key: keyof typeof animatedStats, duration = 2000) => {
            const start = 0
        ...
        >
          <div className="grid md:grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-6 text-center border-ridge bg-white">
              <div className="text-3xl font-bold mb-2">{animatedStats.subreddits}+</div>
              <div className="text-sm">Subreddits Monitored</div>
            </Card>
            <Card className="p-6 text-center border-ridge bg-white">
              <div className="text-3xl font-bold mb-2">{animatedStats.keywords}+</div>
              <div className="text-sm">Keywords Tracked</div>
            </Card>
            <Card className="p-6 text-center border-ridge bg-white">
              <div className="text-3xl font-bold mb-2">{animatedStats.monitoring}/7</div>
